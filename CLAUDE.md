...
- `build-logic/` convention plugins (NowInAndroid pattern), exposed via the version catalog: `mindful.android.application`, `mindful.android.library`, `mindful.android.library.compose`, `mindful.android.feature`, `mindful.android.hilt`, `mindful.android.room`, `mindful.jvm.library`. Android conventions auto-add `testImplementation(projects.core.testing)` (which transitively provides JUnit 4 + MockK + Turbine + coroutines-test) and `androidTestImplementation` (test runner + ext-junit) — modules don't redeclare these. Exception: `mindful.jvm.library` (used by `:core:domain`) adds test deps directly because Android AARs cannot be consumed by JVM modules.
## Module structure
